...
-- -----------------------------------------------------
-- Table `MagicBirdStudio_RBAC`.`UserAuth`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `MagicBirdStudio_RBAC`.`UserAuth` (
  ...
  `Identifier` VARCHAR(45) NULL COMMENT '标识（手机号 邮箱 用户名或第三方应用的唯一标识）',
  `Credential` VARCHAR(45) NULL COMMENT '密码凭证（站内的保存密码，站外的不保存或保存token）',
  PRIMARY KEY (`ID`),
  INDEX `FK_AuthAccont_idx` (`UserID` ASC),
  CONSTRAINT `fk_UserAuth_Account`
    FOREIGN KEY (`UserID`)
    REFERENCES `MagicBirdStudio_RBAC`.`AccountInfo` (`UserID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = '支持第三方登陆的用户验证方式';
